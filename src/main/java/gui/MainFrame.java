...
public class MainFrame extends javax.swing.JFrame {
    ...
    private void exportBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_exportBtnActionPerformed
        ...
        FileUtils.copyDirectory(Statics.RODA_PATH, new File(selectedFolder + "/"
                + Statics.RODA_PATH.getName()));
        JOptionPane.showMessageDialog(rootPane, "تمت عملية التصدير بنجاح للمكان المطلوب");
    }//GEN-LAST:event_exportBtnActionPerformed

    private void importBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_importBtnActionPerformed
        JFileChooser f = new JSystemFileChooser();
        f.setFileSelectionMode(JFileChooser.DIRECTORIES_ONLY);
        f.showOpenDialog(rootPane);

        File selectedFolder = f.getSelectedFile();
        if (selectedFolder == null) {
            return;
        }
        if (".Roda".equals(selectedFolder.getName())) {
            FileUtils.copyDirectory(selectedFolder, Statics.RODA_PATH);
            JOptionPane.showMessageDialog(rootPane, "تم الإستيراد بنجاح\n"
                    + "أعد تشغيل البرنامج للوصول للبيانات بشكل صحيح");
            return;
        } else {
            for (File mabyeFile : selectedFolder.listFiles()) {
                if (".Roda".equals(mabyeFile.getName()) && selectedFolder.isDirectory()) {
                    FileUtils.copyDirectory(mabyeFile, Statics.RODA_PATH);
                    JOptionPane.showMessageDialog(rootPane, "تم الإستيراد بنجاح\n"
                            + "أعد تشغيل البرنامج للوصول للبيانات بشكل صحيح");
                    return;
                }
        ...
        JOptionPane.showMessageDialog(rootPane, "الرجاء التأكد من الملف المراد استيراده");
